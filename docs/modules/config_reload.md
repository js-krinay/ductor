...
- `klir/config_reload.py`: `ConfigReloader`, config diff/classification helpers

## Purpose

Avoid unnecessary restarts when safe `config.json` fields change.

`ConfigReloader` polls `~/.klir/config/config.json` every 5 seconds, validates with `AgentConfig`, diffs top-level schema fields, and:
