...
- `klir/log_context.py`: `ContextVar` state (`agent_name`, `operation`, `chat_id`, `session_id`) + `ContextFilter`.
- `klir/logging_config.py`: root logger setup (console + rotating file via queue listener).
...
- rotating file logs in `~/.klir/logs/agent.log` (`5MB`, `3` backups)
