...
- may inject `GEMINI_API_KEY` from klir config when Gemini settings indicate API-key mode and no env key is set
...
- Gemini:
  ...
  - optional fallback to `~/.klir/config/config.json` `gemini_api_key`

## Model caches

### Codex cache

- file: `~/.klir/config/codex_models.json`
...
- file: `~/.klir/config/gemini_models.json`
