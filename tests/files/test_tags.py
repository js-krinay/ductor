...
class TestPathFromFileTag:
    def test_windows_path_slash_drive_form(self) -> None:
        with patch("klir.files.tags.is_windows", return_value=True):
    ...
    def test_posix_path_unchanged(self) -> None:
        with patch("klir.files.tags.is_windows", return_value=False):
    ...
    def test_non_special_path_not_translated(self) -> None:
        with patch("klir.files.tags.is_windows", return_value=False):
            path = path_from_file_tag("/home/user/.klir/workspace/file.txt")
        assert path == Path("/home/user/.klir/workspace/file.txt")
