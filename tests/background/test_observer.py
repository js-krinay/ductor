...
from klir.background.models import BackgroundResult, BackgroundSubmit
from klir.background.observer import MAX_TASKS_PER_CHAT, BackgroundObserver
...
class TestSubmit:
    async def test_returns_task_id(self, observer: BackgroundObserver) -> None:
        ...
        with patch(
            "klir.background.observer.run_oneshot_task",
...
class TestExecution:
    async def test_success_delivers_result(self, observer: BackgroundObserver) -> None:
        ...
        with patch("klir.background.observer.run_oneshot_task", return_value=result):
