...
from klir.cli.codex_provider import CodexCLI, _codex_final_result, _log_cmd
from klir.cli.executor import SubprocessResult
...
class TestInit:
    def test_find_cli_raises_when_not_on_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("klir.cli.codex_provider.which", lambda _: None)
    ...
    def test_find_cli_uses_resolved_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("klir.cli.codex_provider.which", lambda _: "/opt/bin/codex")
...
class TestEdgeCases:
    ...
    def test_parse_output_error_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.ERROR, logger="klir.cli.codex_provider"):
