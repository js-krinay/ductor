...
from klir.cli.claude_provider import (
    ClaudeCodeCLI,
    _add_opt,
    _log_cmd,
    _parse_response,
...
_EXEC_PATH = "klir.cli.executor.asyncio.create_subprocess_exec"
...
def _make_cli(
    ...
    **kwargs: Any,
    ...
    monkeypatch.setattr("klir.cli.claude_provider.which", lambda _: "/usr/bin/claude")
    cfg = CLIConfig(
        provider="claude",
        model=model,
...
class TestInit:
    def test_find_cli_not_found_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("klir.cli.claude_provider.which", lambda _: None)
...
class TestLogCmd:
    def test_truncates_long_values_after_flags(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="klir.cli.claude_provider"):
            _log_cmd(cmd)
