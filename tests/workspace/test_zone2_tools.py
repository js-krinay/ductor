...
from klir.workspace.init import _walk_and_copy
...
def test_cron_tools_py_files_are_zone2(temp_workspace):
    ...
    _walk_and_copy(home_defaults, klir_home)

    deployed_tool = klir_home / "workspace" / "tools" / "cron_tools" / "cron_add.py"
...
def test_webhook_tools_py_files_are_zone2(temp_workspace):
    ...
    deployed_tool = klir_home / "workspace" / "tools" / "webhook_tools" / "webhook_add.py"
    assert deployed_tool.exists()
    assert deployed_tool.read_text() == "# version 1"
...
def test_user_tools_py_files_are_zone3(temp_workspace):
    ...
    deployed_tool = klir_home / "workspace" / "tools" / "user_tools" / "custom_tool.py"
    assert deployed_tool.exists()
    assert deployed_tool.read_text() == "# version 1"
...
def test_non_py_files_in_tool_dirs_are_zone3(temp_workspace):
    ...
    deployed_config = klir_home / "workspace" / "tools" / "cron_tools" / "config.json"
...
def test_shared_py_is_also_zone2(temp_workspace):
    ...
    deployed_shared = klir_home / "workspace" / "tools" / "cron_tools" / "_shared.py"
