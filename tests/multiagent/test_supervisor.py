...
from klir.multiagent.supervisor import (
...
def main_config(tmp_path: Path) -> AgentConfig:
    """Create a main config with tmp_path as klir_home."""
    return AgentConfig(
        klir_home=str(tmp_path),
...
class TestStartupFailures:
    ...
    async def test_internal_api_start_failure_is_propagated(
        ...
        with (
            patch(
                "klir.multiagent.internal_api.InternalAgentAPI.start",
                new_callable=AsyncMock,
                return_value=False,
            ),
            patch("klir.multiagent.supervisor.AgentStack.create", new_callable=AsyncMock),
...
class TestOnAgentsChanged:
    ...
    async def test_restarts_on_token_change(
        ...
        old_config = AgentConfig(
            telegram_token="old-token", klir_home=str(tmp_path / "agents/sub1")
        )
...
class TestStartSubAgent:
    ...
    async def test_handles_creation_failure(self, supervisor: AgentSupervisor) -> None:
        ...
        with patch(
            "klir.multiagent.supervisor.AgentStack.create",
