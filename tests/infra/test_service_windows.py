...
from klir.infra.service_windows import (
    _TASK_NAME,
    _generate_task_xml,
    _is_access_denied,
    install_service,
...
class TestIsServiceInstalled:
    @patch("klir.infra.service_windows._run_schtasks")
    def test_installed_when_query_succeeds(self, mock_run: MagicMock) -> None:
        mock_run.return_value = make_completed(0, stdout="TaskName: klir")
...
class TestIsServiceRunning:
    ...
    @patch("klir.infra.service_windows._run_schtasks")
    @patch("klir.infra.service_windows.is_service_installed", return_value=True)
    def test_running_when_status_contains_running(
        self, _installed: MagicMock, mock_run: MagicMock
    ) -> None:
        mock_run.return_value = make_completed(0, stdout='"klir","Running","Interactive"')
    ...
    def test_not_running_when_status_says_ready(
        ...
        mock_run.return_value = make_completed(0, stdout='"klir","Ready","Interactive"')
        assert is_service_running() is False
...
class TestInstallService:
    ...
    @patch(
        "klir.infra.service_windows._find_pythonw",
    ...
    @patch("klir.infra.service_windows.is_service_available", return_value=False)
...
class TestStopService:
    @patch("klir.infra.service_windows._run_schtasks")
    @patch("klir.infra.service_windows.is_service_running", return_value=True)
    ...
    @patch("klir.infra.service_windows.is_service_running", return_value=False)
...
class TestPrintServiceLogs:
    ...
    @patch("klir.infra.service_windows.resolve_paths")
    @patch("klir.infra.service_windows.is_service_installed", return_value=True)
