...
from klir.infra.install import detect_install_mode, is_upgradeable
...
class TestDetectInstallMode:
    ...
    def test_editable_install_detected_as_dev(self) -> None:
        ...
        with (
            patch("klir.infra.install.sys") as mock_sys,
            patch("klir.infra.install.distribution", return_value=mock_dist),
        ):
    ...
    def test_package_not_found_is_dev(self) -> None:
        ...
        with (
            patch("klir.infra.install.sys") as mock_sys,
            patch(
                "klir.infra.install.distribution",
                side_effect=PackageNotFoundError("klir"),
            ),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_metadata_error_falls_back_to_dev(self) -> None:
        with (
            patch("klir.infra.install.sys") as mock_sys,
            patch("klir.infra.install.distribution", side_effect=OSError("corrupt")),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_pipx_path_variant_windows(self) -> None:
        with patch("klir.infra.install.sys") as mock_sys:
            mock_sys.prefix = "C:\\Users\\me\\AppData\\Local\\pipx\\venvs\\klir"
            assert detect_install_mode() == "pipx"
...
class TestIsUpgradeable:
    ...
    def test_pip_is_upgradeable(self) -> None:
        with patch("klir.infra.install.detect_install_mode", return_value="pip"):
            assert is_upgradeable() is True
