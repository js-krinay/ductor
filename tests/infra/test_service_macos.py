...
from klir.infra.service_macos import (
    _LABEL,
...
class TestGeneratePlistData:
    def test_contains_binary_path(self) -> None:
        data = _generate_plist_data("/usr/local/bin/klir")
        assert data["ProgramArguments"] == ["/usr/local/bin/klir"]

    def test_has_label(self) -> None:
        data = _generate_plist_data("klir")
        assert data["Label"] == "dev.klir"
    ...
    def test_path_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("klir.infra.service_macos.Path.home", return_value=tmp_path):
            data = _generate_plist_data("klir")
...
class TestIsServiceRunning:
    ...
    @patch("klir.infra.service_macos.is_service_installed", return_value=True)
...
class TestInstallService:
    ...
    def test_install_success(
        ...
        plist_file = tmp_path / "dev.klir.plist"
        ...
        assert plist_data["Label"] == "dev.klir"

    @patch("klir.infra.service_macos.is_service_available", return_value=False)
...
class TestStopService:
    @patch("klir.infra.service_macos._run_launchctl")
    @patch("klir.infra.service_macos.is_service_running", return_value=True)
    ...
    @patch("klir.infra.service_macos.is_service_running", return_value=False)
