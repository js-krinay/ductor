...
class TestPerformUpgradePipeline:
    ...
    async def test_changes_on_first_attempt(self) -> None:
        with (
            patch(
                "klir.infra.updater._perform_upgrade_impl",
                new=AsyncMock(return_value=(True, "first-pass")),
            ) as mock_upgrade,
            patch(
                "klir.infra.updater._wait_for_version_change",
    ...
    async def test_returns_unchanged_when_no_retry_target(self) -> None:
        with (
            ...
            patch(
                "klir.infra.updater._resolve_retry_target",
...
class TestUpdateObserver:
    ...
    async def test_notifies_on_new_version(self) -> None:
        ...
        with (
            patch("klir.infra.updater.check_pypi", return_value=info),
            patch("klir.infra.updater._INITIAL_DELAY_S", 0),
            patch("klir.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_handles_check_failure_gracefully(self) -> None:
        ...
        with (
            patch("klir.infra.updater.check_pypi", side_effect=RuntimeError("network")),
            patch("klir.infra.updater._INITIAL_DELAY_S", 0),
            patch("klir.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_handles_none_from_check_pypi(self) -> None:
        ...
        with (
            patch("klir.infra.updater.check_pypi", return_value=None),
            patch("klir.infra.updater._INITIAL_DELAY_S", 0),
            patch("klir.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_notifies_again_for_newer_version(self) -> None:
        ...
        with (
            patch("klir.infra.updater.check_pypi", side_effect=mock_check),
            patch("klir.infra.updater._INITIAL_DELAY_S", 0),
            patch("klir.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
