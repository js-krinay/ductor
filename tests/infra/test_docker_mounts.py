...
class TestDockerMountCLI:
    ...
    def test_mount_adds_path(self, tmp_path: Path) -> None:
        from klir.cli_commands.docker import docker_mount
        ...
        with patch("klir.cli_commands.docker.resolve_paths", return_value=paths_mock):
    ...
    def test_unmount_removes_path(self, tmp_path: Path) -> None:
        from klir.cli_commands.docker import docker_unmount
    ...
    def test_mounts_list_empty(self, tmp_path: Path) -> None:
        from klir.cli_commands.docker import docker_list_mounts
...
class TestConfigDeepMergeMounts:
    ...
    def test_new_mounts_key_added(self) -> None:
        from klir.config import deep_merge_config
